...
def _get_dashscope_adapter():
    ...
    with _adapter_lock:
        if _DASHSCOPE_ADAPTER is None:
            try:
                from stock_agent.llm_adapters.dashscope_adapter import ChatDashScope
...
def _get_openai_responses_adapter():
    ...
    with _adapter_lock:
        if _OPENAI_RESPONSES_ADAPTER is None:
            try:
                from stock_agent.llm_adapters.openai_responses_adapter import ChatOpenAIResponses
...
def _get_deepseek_reasoner_adapter():
    ...
    with _adapter_lock:
        if _DEEPSEEK_REASONER_ADAPTER is None:
            try:
                from stock_agent.llm_adapters.deepseek_reasoner_adapter import ChatDeepSeekReasoner
